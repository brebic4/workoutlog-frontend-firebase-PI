...
<template>
  ...
  >
    <div
      class="bg-white w-full max-w-md rounded-2xl shadow-xl p-6 text-gray-900 dark:bg-gray-900 dark:text-gray-100 border border-transparent dark:border-gray-700"
    >
      <div class="flex items-start justify-between gap-4">
        <h2 class="text-xl font-bold">Promijeni lozinku</h2>
        <button
          class="text-gray-500 cursor-pointer hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200"
          @click="close"
        >
          ✕
        </button>
      ...
      <div class="mt-4 space-y-3">
        <div>
          <label class="block text-sm text-gray-600 dark:text-gray-300 mb-1">Nova lozinka</label>

          <div class="relative">
            <input
              v-model="newPassword"
              :type="showNew ? 'text' : 'password'"
              class="w-full border rounded-xl px-3 py-2 pr-12 bg-white text-gray-900 dark:bg-gray-800 dark:text-gray-100 dark:border-gray-600"
              autocomplete="new-password"
            />

            <button
              type="button"
              class="absolute right-2 top-1/2 -translate-y-1/2 p-2 rounded-lg cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700"
              @click="toggleNew"
          ...
          <div v-if="newPassword" class="mt-2 space-y-2">
            ...
            <ul class="text-xs space-y-1 dark:text-gray-300">
              <li
                :class="
                  passwordChecks.minLength ? 'text-green-600' : 'text-gray-500 dark:text-gray-400'
                "
              >
                • najmanje 8 znakova
              </li>
              ...
              <li
                :class="
                  passwordChecks.hasNumber ? 'text-green-600' : 'text-gray-500 dark:text-gray-400'
                "
              >
                • broj
              </li>
        ...
        <p v-if="error" class="text-sm text-red-600 dark:text-red-400 whitespace-pre-line">
          {{ error }}
        </p>
